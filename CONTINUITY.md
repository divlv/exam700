...
- **Constraints/Assumptions:**
  ...
  - `--forwarded-allow-ips '*'` обязателен в команде uvicorn: без него
    ...
    `always_trust`).
  - **КРИТИЧНО, проверено на живом кластере пользователя (реальный краш пода
    в `CrashLoopBackOff`)**: `readOnlyRootFilesystem: true` делает `/app`
    (и дефолтный `<PROJECT_ROOT>/logs`) недоступным для записи. Без явного
    `AZ700_LOGS_DIR` первый же `get_logger()` при импорте падает
    `PermissionError`/`OSError [Errno 30] Read-only file system` ещё до
    старта uvicorn. Исправлено: `AZ700_LOGS_DIR=/data/az700/logs`
    прописан и в `Dockerfile` (ENV по умолчанию), и явно в
    `deploy/k3s/az700-app.yaml` (для самодокументирования рядом с
    `AZ700_DATA_DIR`). **Локальные дымовые тесты этого не поймали**,
    потому что без Docker `<repo>/logs` на Windows всегда был доступен для
    записи — баг проявляется только при реальном
    `readOnlyRootFilesystem`. Перепроверено воспроизведением: зеркальная
    копия `/app` с `icacls … /deny (WD,AD)` (запрет только записи, чтение и
    listing разрешены — так же, как реально ведёт себя k8s) — без фикса
    падает с тем же `PermissionError`, с фиксом стартует и обслуживает
    запросы чисто.
...
- **Now:** Пользователь уже применил первую версию манифестов и получил
  `CrashLoopBackOff` — под падал при импорте из-за
  `readOnlyRootFilesystem` + недостижимого `<PROJECT_ROOT>/logs`. Причина
  найдена, исправлена в `Dockerfile` и `deploy/k3s/az700-app.yaml`
  (`AZ700_LOGS_DIR=/data/az700/logs`), исправление воспроизведено и
  перепроверено локально (см. Constraints). Пользователю нужно применить
  обновлённый `az700-app.yaml` и перезапустить под — новый образ для этого
  не обязателен, т.к. переменная окружения задаётся в самом манифесте.

- **Next:**
  1. Пользователь: `k3s kubectl apply -f az700-app.yaml` (обновлённая
     версия с `AZ700_LOGS_DIR`) + `k3s kubectl -n mywebs rollout restart
     deployment/az700`, либо просто переприменить весь `install.sh`.
  2. Дождаться `1/1 Running`, проверить `/healthz` и вход под `dima`.
  3. Остальное по `docs/deploy-k3s.md` без изменений: DNS `az700.v1.lv`,
     датасет на сервере, `az700-secret.yaml`, публичность пакета GHCR.
  Возможные улучшения по запросу: перенести дополнительные предпочтения
  дизайна, добавить экспорт статистики.
